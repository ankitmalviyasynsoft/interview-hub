...
export const CustomerProfileComponent = () => {
    ...
    const { control, handleSubmit, register, formState: { errors } } = useForm<CustomerProfileFormValues>({
    ...
    return (
        <div className="w-full max-w-5xl mx-auto pb-16 animate-in fade-in slide-in-from-bottom-8 duration-700">
            {/* --- Cover Image --- */}
            <div className="relative h-64 w-full rounded-b-[2.5rem] bg-gradient-to-br from-violet-600 via-indigo-600 to-purple-700 shadow-xl overflow-visible">
            ...
            <div className="px-4 sm:px-8 lg:px-12 relative z-10 -mt-20">
                ...
                {/* --- Unified Header Card --- */}
                <div className="bg-white rounded-[2rem] shadow-xl shadow-slate-200/60 p-6 sm:p-8 flex flex-col md:flex-row items-center md:items-end gap-6 mb-8 border border-slate-100">

                    {/* Avatar - Positioned to float slightly above */}
                    <div className="relative -mt-20 md:-mt-24 shrink-0">
                        <div className="h-36 w-36 rounded-[2rem] rotate-3 border-4 border-white bg-white shadow-2xl overflow-hidden flex items-center justify-center relative z-10 transition-transform duration-300 hover:rotate-0">
                            <div className="absolute inset-0 bg-gradient-to-br from-slate-100 to-slate-200"></div>
                            <span className="text-4xl font-black text-indigo-600/80 relative z-10 select-none">
                                {displayData.fullName.split(' ').map(n => n[0]).join('')}
                            </span>
                        </div>
                        {/* Avatar Decorator */}
                        <div className="absolute inset-0 bg-indigo-500 rounded-[2rem] rotate-6 opacity-20 scale-95 z-0"></div>

                        <button
                            className="absolute -bottom-2 -right-2 z-20 p-2.5 rounded-xl bg-indigo-600 text-white shadow-lg hover:bg-indigo-700 hover:scale-110 active:scale-95 transition-all duration-200"
                            title="Update Photo"
                        >
                    ...
                    <div className="pb-1">
                        <Button
                            ...
                            className={`shadow-lg shadow-indigo-500/20 rounded-full px-6 transition-all duration-300 ${isEditing ? "bg-slate-100 hover:bg-slate-200 text-slate-700" : "bg-indigo-600 hover:bg-indigo-700 hover:-translate-y-0.5"}`}
                        >
                            {isEditing ? "Cancel" : (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="lg:col-span-4 space-y-6">
                        ...
                        <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
                            <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-4">Profile Strength</h3>
                        ...
                        <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider">Skills</h3>
                                <button className="text-indigo-600 hover:text-indigo-700 text-sm font-medium transition-colors">+ Add</button>
                            </div>
                            <div className="flex flex-wrap gap-2">
                                {["React", "TypeScript", "Node.js", "Design System", "Next.js", "PostgreSQL"].map((skill, i) => (
                                    <span key={i} className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-slate-50 text-slate-600 border border-slate-200 hover:border-indigo-200 hover:bg-indigo-50 hover:text-indigo-600 transition-all cursor-default">
                                        {skill}
                                    </span>
                                ))}
                            </div>
                    ...
                    <div className="lg:col-span-8">
                        <div className="bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden min-h-[500px]">
                            {/* Tabs Header */}
                            <div className="flex flex-wrap border-b border-slate-100 px-8 pt-6 pb-0 gap-8">
                                <button
                                    onClick={() => setActiveTab("personal")}
                                    className={`pb-4 text-sm font-semibold border-b-2 transition-colors ${activeTab === "personal" ? "border-indigo-600 text-indigo-600" : "border-transparent text-slate-400 hover:text-slate-600"}`}
                                >
                                    Personal Details
                                </button>
                                <button
                                    onClick={() => setActiveTab("account")}
                                    className={`pb-4 text-sm font-semibold border-b-2 transition-colors ${activeTab === "account" ? "border-indigo-600 text-indigo-600" : "border-transparent text-slate-400 hover:text-slate-600"}`}
                                >
                                    Account Settings
                                </button>
                                <button
                                    onClick={() => setActiveTab("notifications")}
                                    className={`pb-4 text-sm font-semibold border-b-2 transition-colors ${activeTab === "notifications" ? "border-indigo-600 text-indigo-600" : "border-transparent text-slate-400 hover:text-slate-600"}`}
                                >
                                    Notifications
                                </button>
                            </div>
                            ...
                            <div className="p-8">
                                            ...
                                            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
                                                    <ControlledInput
                                                        ...
                                                        control={control}
                                                        label="Full Name"
                                                        className="bg-slate-50 border-slate-200 focus:border-indigo-500 focus:bg-white"
                                                    />
                                                    <ControlledInput
                                                        name="headline"
                                                        control={control}
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    <textarea
                                                        {...register("bio")}
                                                        className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 focus:bg-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none transition-all duration-200 text-slate-900 placeholder:text-slate-400 min-h-[140px] resize-y"
                                                        placeholder="Tell us about yourself..."
                                                    />
                                                </div>

                                                <div className="flex justify-end pt-4 border-t border-slate-100">
                                                    <Button type="submit" size="lg" className="px-8 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg shadow-indigo-500/30">
                                                        Save Changes
                                                    </Button>
                                                </div>
                                            </form>
                                ...
                                {activeTab === "account" && (
                                    <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in slide-in-from-bottom-2">
                                        ...
                                        <p className="text-slate-500 max-w-xs mt-2">Manage your password, login sessions, and linked accounts here.</p>
                                        <Button variant="outline" className="mt-6" onClick={() => { }}>Coming Soon</Button>
                                    </div>
                                )}
                                ...
                                {activeTab === "notifications" && (
                                    <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in slide-in-from-bottom-2">
                                        ...
                                        <p className="text-slate-500 max-w-xs mt-2">Customize how and when you want to receive updates from us.</p>
                                        <Button variant="outline" className="mt-6" onClick={() => { }}>Coming Soon</Button>
                                    </div>
                                )}
                            </div>
...
const InfoItem = ({ icon, label, value, isLink }: { icon: React.ReactNode, label: string, value: string, isLink?: boolean }) => (
    <div className="flex items-start gap-4 p-4 rounded-2xl bg-slate-50/50 border border-slate-100/50 hover:bg-white hover:shadow-md hover:scale-[1.02] transition-all duration-300 group">
        <div className="p-3 rounded-xl bg-white text-indigo-600 shadow-sm border border-slate-100 group-hover:border-indigo-100 group-hover:text-indigo-700 transition-colors">
            {icon}
        </div>
        <div>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-0.5">{label}</p>
            {isLink ? (
                <a href={value} target="_blank" rel="noreferrer" className="text-slate-900 font-medium hover:text-indigo-600 hover:underline transition-colors truncate block">
                    {value.replace(/^https?:\/\//, '')}
                </a>
            ) : (
